...
static MOBIXmlState * mobi_xml_state_init(const char *name, const MOBI_XML_MODE mode) {
    MOBIXmlState *curr = (MOBIXmlState *)calloc(1, sizeof(MOBIXmlState));
    if (curr == NULL) {
        return NULL;
    }
...
static MOBIXmlState * mobi_xml_state_push(xmlTextWriterPtr writer, const char *name, const MOBI_XML_MODE mode) {
    MOBIXmlState *nw = mobi_xml_state_init(name, mode);
...
xmlBufferPtr xmlBufferCreate(void) {
    ...
    xmlbuf = (xmlBufferPtr)malloc(sizeof(xmlBuffer));
    if (xmlbuf == NULL) {
...
xmlTextWriterPtr xmlNewTextWriterMemory(xmlBufferPtr xmlbuf, int compression) {
    ...
    writer = (xmlTextWriterPtr)malloc(sizeof(xmlTextWriter));
    if (writer == NULL) {
...
int xmlTextWriterStartAttributeNS(xmlTextWriterPtr writer,
    ...
    if (namespaceURI != NULL) {
        char xmlns[] =  "xmlns";
        if (prefix != NULL) {
            size_t length = sizeof(xmlns) - 1 + strlen((char *) prefix) + 1; /* add one for ":" */
            char *prefixed = (char *)malloc(length + 1);
            if (prefixed == NULL) {
                debug_print("%s\n", "Memory allocation failed");
                return XML_ERROR;
            }
            sprintf(prefixed, "%s:%s", xmlns, prefix);
            ...
            MOBI_RET ret = mobi_xml_save_ns(writer, xmlns, (char *) namespaceURI);
            if (ret != MOBI_SUCCESS) {
                return XML_ERROR;
            }
        }
...
int xmlTextWriterStartElementNS(xmlTextWriterPtr writer,
    ...
    if (namespaceURI != NULL) {
        char xmlns[] =  "xmlns";
        if (prefix != NULL) {
            size_t length = sizeof(xmlns) - 1 + strlen((char *) prefix) + 1;
            char *prefixed = (char *)malloc(length + 1);
            if (prefixed == NULL) {
                debug_print("%s\n", "Memory allocation failed");
                return XML_ERROR;
