...
MOBIBuffer * buffer_init_null(unsigned char *data, const size_t len) {
    MOBIBuffer *buf = (MOBIBuffer *)malloc(sizeof(MOBIBuffer));
	if (buf == NULL) {
...
void buffer_resize(MOBIBuffer *buf, const size_t newlen) {
    unsigned char *tmp = (unsigned char *)realloc(buf->data, newlen);
    if (tmp == NULL) {
        debug_print("%s", "Buffer allocation failed\n");
...
void buffer_dup8(uint8_t **val, MOBIBuffer *buf) {
    ...
    *val = (uint8_t *)malloc(sizeof(uint8_t));
    if (*val == NULL) {
        return;
    }
    **val = buffer_get8(buf);
...
void buffer_dup16(uint16_t **val, MOBIBuffer *buf) {
    ...
    *val = (uint16_t *)malloc(sizeof(uint16_t));
    if (*val == NULL) {
        return;
    }
