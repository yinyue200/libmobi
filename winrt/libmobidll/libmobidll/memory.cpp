...
MOBIData * mobi_init(void) {
	MOBIData *m = NULL;
	m = (MOBIData *)calloc(1, sizeof(MOBIData));
...
MOBIHuffCdic * mobi_init_huffcdic(void) {
	MOBIHuffCdic *huffcdic = (MOBIHuffCdic *)calloc(1, sizeof(MOBIHuffCdic));
	if (huffcdic == NULL) {
...
MOBIRawml * mobi_init_rawml(const MOBIData *m) {
	MOBIRawml *rawml = (MOBIRawml *)malloc(sizeof(MOBIRawml));
	if (rawml == NULL) {
...
MOBIIndx * mobi_init_indx(void) {
	MOBIIndx *indx = (MOBIIndx *)calloc(1, sizeof(MOBIIndx));
	if (indx == NULL) {
