...
MOBIArray * array_init(const size_t len) {
    MOBIArray *arr = NULL;
    arr = (MOBIArray *)malloc(sizeof(MOBIArray));
    if (arr == NULL) {
        debug_print("%s", "Array allocation failed\n");
        return NULL;
    }
    arr->data = (uint32_t *)malloc(len * sizeof(*arr->data));
    if (arr->data == NULL) {
        free(arr);
...
MOBI_RET array_insert(MOBIArray *arr, const uint32_t value) {
    ...
    if (arr->maxsize == arr->size) {
        ...
        uint32_t *tmp = (uint32_t *)realloc(arr->data, arr->maxsize * sizeof(*arr->data));
        if (!tmp) {
            free(arr->data);
            arr->data = NULL;
...
static MOBITrie * mobi_trie_mknode(void) {
    MOBITrie *node = (MOBITrie *)calloc(1, sizeof(MOBITrie));
    if (node == NULL) {
...
static MOBI_RET mobi_trie_addvalue(MOBITrie *node, char *value) {
    if (node->values) {
        ...
        node->values = (void**)new_values;
        node->values[cnt - 1] = value;
    } else {
        node->values = (void**)malloc(sizeof(*node->values));
        if (node->values == NULL) {
            debug_print("Memory allocation failed%s", "\n");
            return MOBI_MALLOC_FAILED;
        }
...
static MOBIFragment * mobi_list_init(size_t raw_offset, unsigned char *fragment, const size_t size, const bool is_malloc) {
    MOBIFragment *curr = (MOBIFragment *)calloc(1, sizeof(MOBIFragment));
    if (curr == NULL) {
        if (is_malloc) {
            free(fragment);
        }
...
MOBIFragment * mobi_list_add(MOBIFragment *curr, size_t raw_offset, unsigned char *fragment, const size_t size, const bool is_malloc) {
    ...
    curr->next = (MOBIFragment *)calloc(1, sizeof(MOBIFragment));
    if (curr->next == NULL) {
        if (is_malloc) {
            free(fragment);
        }
...
MOBIFragment * mobi_list_insert(MOBIFragment *curr, size_t raw_offset, unsigned char *fragment, const size_t size, const bool is_malloc, const size_t offset) {
    ...
    MOBIFragment *nw = (MOBIFragment *)calloc(1, sizeof(MOBIFragment));
    if (nw == NULL) {
        if (is_malloc) {
            free(fragment);
        }
	...
	nw->raw_offset = raw_offset;
	nw->fragment = fragment;
	nw->size = size;
	nw->is_malloc = is_malloc;
    ...
    if (curr->raw_offset == offset) {
        ...
        if (prev) {
            ...
            tmp.next = curr->next;
            /* move new to curr */
            curr->raw_offset = nw->raw_offset;
            curr->fragment = nw->fragment;
            curr->size = nw->size;
            curr->is_malloc = nw->is_malloc;
            curr->next = nw;
            /* restore tmp to new */
			nw->raw_offset = tmp.raw_offset;
			nw->fragment = tmp.fragment;
			nw->size = tmp.size;
			nw->is_malloc = tmp.is_malloc;
			nw->next = tmp.next;
            return curr;
        }
        ...
        new2 = (MOBIFragment *)calloc(1, sizeof(MOBIFragment));
        if (new2 == NULL) {
            free(nw);
            if (is_malloc) {
                free(fragment);
