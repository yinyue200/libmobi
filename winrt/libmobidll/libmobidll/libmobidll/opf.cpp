...
MOBI_RET mobi_build_opf_guide(OPF *opf, const MOBIRawml *rawml) {
    ...
    opf->guide = (OPFguide*)malloc(sizeof(OPFguide));
    if (opf->guide == NULL) {
        debug_print("%s\n", "Memory allocation failed");
        return MOBI_MALLOC_FAILED;
    }
    OPFreference **reference = (OPFreference **)malloc((count + 1) * sizeof(OPFreference*));
    if (reference == NULL) {
        free(opf->guide);
        opf->guide = NULL;
        debug_print("%s\n", "Memory allocation failed");
    ...
    while (i < count) {
        ...
        if (!mobi_is_guide_type(type)) {
            ...
            type_size += 6;
            ref_type = (char *)malloc(type_size + 1);
            if (ref_type == NULL) {
                free(reference);
                free(opf->guide);
                opf->guide = NULL;
        ...
        reference[j] = (OPFreference *)calloc(1, sizeof(OPFreference));
        *reference[j] = { ref_type, ref_title, ref_href };
        i++;
        j++;
    }
...
MOBI_RET mobi_opf_add_to_rawml(const char *opf_xml, MOBIRawml *rawml) {
    ...
    if (rawml->resources) {
        ...
        uid = part->uid + 1;
        part->next = (MOBIPart *)calloc(1, sizeof(MOBIPart));
        opf_part = part->next;
    }
    else {
        rawml->resources = (MOBIPart *)calloc(1, sizeof(MOBIPart));
...
MOBI_RET mobi_build_ncx(MOBIRawml *rawml, const OPF *opf) {
    ...
    if (rawml->ncx && rawml->ncx->cncx_record) {
        ...
        NCX *ncx = (NCX *)malloc(count * sizeof(NCX));
        if (ncx == NULL) {
            debug_print("%s\n", "Memory allocation failed");
            return MOBI_MALLOC_FAILED;
        }
        while (i < count) {
            ...
            char *target = (char *)malloc(MOBI_ATTRNAME_MAXSIZE + 1);
            if (target == NULL) {
                free(text);
                mobi_free_ncx(ncx, i);
                debug_print("%s\n", "Memory allocation failed");
            ...
            ncx[i++] = {id, text, target, level, parent, first_child, last_child};
        }
...
static void mobi_opf_fill_tag(const MOBIData *m, const MOBIExthHeader *exth, char ***array) {
    if (*array == NULL) {
        *array = (char **)calloc(OPF_META_MAX_TAGS, sizeof(**array));
        if (*array == NULL) {
            return;
        }
    }
    ...
    while (i < OPF_META_MAX_TAGS) {
        ...
        if (exth_tag.type == EXTH_NUMERIC) {
            value = (char *)malloc(10 + 1);
            if (value) {
                const uint32_t val32 = mobi_decode_exthvalue((unsigned char*)exth->data, exth->size);
                snprintf(value, 10 + 1, "%d", val32);
            }
        } else if (exth_tag.type == EXTH_STRING) {
            value = mobi_decode_exthstring(m, (unsigned char*)exth->data, exth->size);
        }
        if (value) {
            (*array)[i] = value;
        }
...
static void mobi_opf_set_meta(OPFmeta **meta, const char *name, const char *content) {
    ...
    while (i < OPF_META_MAX_TAGS) {
        ...
        meta[i] = (OPFmeta*)malloc(sizeof(OPFmeta));
        if (meta[i] == NULL) {
            return;
        }
...
static void mobi_opf_copy_meta(const MOBIData *m, const MOBIExthHeader *exth, OPFmeta **meta, const char *name) {
    ...
    if (exth_tag.tag == EXTH_COVEROFFSET) {
        content = (char*)malloc(13 + 1);
        if (content) {
            const uint32_t val32 = mobi_decode_exthvalue((unsigned char*)exth->data, exth->size);
            snprintf(content, 14, "resource%05d", val32);
        }
    } else if (exth_tag.type == EXTH_NUMERIC) {
        content = (char *)malloc(10 + 1);
        if (content) {
            const uint32_t val32 = mobi_decode_exthvalue((unsigned char*)exth->data, exth->size);
            snprintf(content, 11, "%d", val32);
        }
    } else if (exth_tag.type == EXTH_STRING) {
        char *string = mobi_decode_exthstring(m, (unsigned char*)exth->data, exth->size);
        content = string;
    }
    if (content) {
...
#define mobi_opf_copy_tagtype(mobidata, struct_type, struct_element, member_name) { \
    if (struct_element == NULL) { \
        struct_element = (struct_type **)calloc(OPF_META_MAX_TAGS, sizeof(*struct_element)); \
        if(struct_element == NULL) { \
            debug_print("%s\n", "Memory allocation failed"); \
            return MOBI_MALLOC_FAILED; \
        } \
    ...
    while (i < OPF_META_MAX_TAGS) { \
        ...
        if(element[i] != NULL) { \
			...
			auto vdp=calloc(1, sizeof(*element[i])); \
            element[i] = (struct_type *)vdp; \
            if(element[i] == NULL) { \
                debug_print("%s\n", "Memory allocation failed"); \
                return MOBI_MALLOC_FAILED; \
            } \
        ...
        if (exth_tag.type == EXTH_NUMERIC) { \
            value = (char *)malloc(10 + 1); \
            if (value) { \
                const uint32_t val32 = mobi_decode_exthvalue((unsigned char*)curr->data, curr->size); \
                snprintf(value, 10 + 1, "%d", val32); \
            } else { \
                error_ret = MOBI_MALLOC_FAILED; \
            } \
        } else if (exth_tag.type == EXTH_STRING) { \
            value = mobi_decode_exthstring((MOBIData *)mobidata, (unsigned char*)curr->data, curr->size); \
        } \
        if(value == NULL) { \
            free(element[i]); \
            element[i] = NULL; \
...
MOBI_RET mobi_build_opf_metadata(OPF *opf,  const MOBIData *m, const MOBIRawml *rawml) {
    ...
    opf->metadata = (OPFmetadata*)calloc(1, sizeof(OPFmetadata));
    if (opf->metadata == NULL) {
        debug_print("%s\n", "Memory allocation failed");
        return MOBI_MALLOC_FAILED;
    }
    /* initialize metadata sub-elements */
    opf->metadata->meta = (OPFmeta**)calloc(OPF_META_MAX_TAGS, sizeof(OPFmeta*));
    if (opf->metadata->meta == NULL) {
        debug_print("%s\n", "Memory allocation failed");
        return MOBI_MALLOC_FAILED;
    }
    opf->metadata->dc_meta = (OPFdcmeta*)calloc(1, sizeof(OPFdcmeta));
    if (opf->metadata->dc_meta == NULL) {
        debug_print("%s\n", "Memory allocation failed");
        return MOBI_MALLOC_FAILED;
    }
    opf->metadata->x_meta = (OPFxmeta*)calloc(1, sizeof(OPFxmeta));
    ...
    if (opf->metadata->dc_meta->title == NULL) {
        opf->metadata->dc_meta->title = (char **)calloc(OPF_META_MAX_TAGS, sizeof(char*));
        if (opf->metadata->dc_meta->title == NULL) {
            debug_print("%s\n", "Memory allocation failed");
            return MOBI_MALLOC_FAILED;
        }
    ...
    if (opf->metadata->dc_meta->language == NULL) {
        opf->metadata->dc_meta->language = (char **)calloc(OPF_META_MAX_TAGS, sizeof(char*));
        if (opf->metadata->dc_meta->language == NULL) {
            debug_print("%s\n", "Memory allocation failed");
            return MOBI_MALLOC_FAILED;
        }
    ...
    if (mobi_is_dictionary(m)) {
        if (opf->metadata->x_meta->dictionary_in_lang == NULL) {
            if (m->mh && m->mh->dict_input_lang) {
                opf->metadata->x_meta->dictionary_in_lang = (char **)calloc(OPF_META_MAX_TAGS, sizeof(char*));
                if (opf->metadata->x_meta->dictionary_in_lang == NULL) {
                    debug_print("%s\n", "Memory allocation failed");
                    return MOBI_MALLOC_FAILED;
                }
        ...
        if (opf->metadata->x_meta->dictionary_out_lang == NULL) {
            if (m->mh && m->mh->dict_output_lang) {
                opf->metadata->x_meta->dictionary_out_lang = (char **)calloc(OPF_META_MAX_TAGS, sizeof(char*));
                if (opf->metadata->x_meta->dictionary_out_lang == NULL) {
                    debug_print("%s\n", "Memory allocation failed");
                    return MOBI_MALLOC_FAILED;
                }
        ...
        if (rawml->orth->orth_index_name) {
            opf->metadata->x_meta->default_lookup_index = (char **)calloc(OPF_META_MAX_TAGS, sizeof(char*));
...
MOBI_RET mobi_xml_write_element_ns(xmlTextWriterPtr writer, const char *name, const char **content, const char *ns) {
    if (content) {
        ...
        while (i < OPF_META_MAX_TAGS) {
            ...
            int xml_ret = xmlTextWriterWriteElementNS(writer, xmlns, BAD_CAST name, NULL, BAD_CAST content[i]);
            if (xml_ret < 0) {
