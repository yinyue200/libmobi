...
static MOBI_RET mobi_parse_ordt(MOBIBuffer *buf, MOBIOrdt *ordt) {
    ...
    if (buffer_match_magic(buf, ORDT_MAGIC)) {
        ...
        ordt->ordt1 = (uint8_t *)malloc(ordt->offsets_count * sizeof(*ordt->ordt1));
        if (ordt->ordt1 == NULL) {
    ...
    if (buffer_match_magic(buf, ORDT_MAGIC)) {
        ...
        ordt->ordt2 = (uint16_t *)malloc(ordt->offsets_count * sizeof(*ordt->ordt2));
        if (ordt->ordt2 == NULL) {
...
static MOBI_RET mobi_parse_tagx(MOBIBuffer *buf, MOBITagx *tagx) {
    ...
    tagx->tags = (TAGXTags *)malloc(tagx_record_length * sizeof(TAGXTags));
    if (tagx->tags == NULL) {
...
static MOBI_RET mobi_parse_index_entry(MOBIIndx *indx, const MOBIIdxt idxt, const MOBITagx *tagx, const MOBIOrdt *ordt, MOBIBuffer *buf, const size_t curr_number) {
    ...
    indx->entries[entry_number].label = (char *)malloc(label_length + 1);
    ...
    if (tagx->tags_count > 0) {
        ...
        MOBIPtagx *ptagx = (MOBIPtagx *)malloc(tagx->tags_count * sizeof(MOBIPtagx));
        if (ptagx == NULL) {
        ...
        indx->entries[entry_number].tags = (MOBIIndexTag *)malloc(tagx->tags_count * sizeof(MOBIIndexTag));
        ...
        while (i < ptagx_count) {
            ...
            if (tagvalues_count) {
                ...
                indx->entries[entry_number].tags[i].tagvalues = (uint32_t *)malloc(arr_size);
    ...
    if (buffer_match_magic_offset(buf, TAGX_MAGIC, header_length) && indx->total_entries_count == 0) {
        ...
        if (index_name_offset > 0 && index_name_length > 0) {
            if (index_name_length <= header_length - index_name_offset && index_name_length < INDX_NAME_SIZEMAX) {
                ...
                char *name = (char *)malloc(index_name_length + 1);
                if (name == NULL) {
        ...
        indx->encoding = (MOBIEncoding)encoding;
        indx->type = type;
        ...
        MOBIIdxt idxt;
        uint32_t *offsets = (uint32_t *)malloc((entries_count + 1) * sizeof(uint32_t));
        if (offsets == NULL) {
            buffer_free_null(buf);
            debug_print("%s\n", "Memory allocation failed");
            return MOBI_MALLOC_FAILED;
        ...
        if (entries_count > 0) {
            if (indx->entries == NULL) {
                indx->entries = (MOBIIndexEntry *)malloc(indx->total_entries_count * sizeof(MOBIIndexEntry));
                if (indx->entries == NULL) {
                    buffer_free_null(buf);
                    free(offsets);
                    debug_print("%s\n", "Memory allocation failed");
...
MOBI_RET mobi_parse_index(const MOBIData *m, MOBIIndx *indx, const size_t indx_record_number) {
    ...
    MOBITagx *tagx = (MOBITagx *)calloc(1, sizeof(MOBITagx));
    if (tagx == NULL) {
        mobi_free_indx(indx);
        debug_print("%s\n", "Memory allocation failed");
        return MOBI_MALLOC_FAILED;
    ...
    MOBIOrdt *ordt = (MOBIOrdt *)calloc(1, sizeof(MOBIOrdt));
    if (ordt == NULL) {
        mobi_free_indx(indx);
        mobi_free_tagx(tagx);
        debug_print("%s\n", "Memory allocation failed");
...
MOBI_RET mobi_get_indxentry_tagvalue(uint32_t *tagvalue, const MOBIIndexEntry *entry, const Initls& tag_arr) {
	auto bg = tag_arr.list.begin();
	unsigned temp[]{ *bg,*(++bg) };
	return mobi_get_indxentry_tagvalue(tagvalue, entry, temp);
}
/**
...
char * mobi_get_cncx_string_utf8(const MOBIPdbRecord *cncx_record, const uint32_t cncx_offset, MOBIEncoding cncx_encoding) {
    ...
    if (string != NULL && cncx_encoding == MOBI_CP1252) {
        ...
        char *decoded = (char *)malloc(out_len);
        if (decoded) {
