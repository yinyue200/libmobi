...
MOBI_RET mobi_load_pdbheader(MOBIData *m, Libmobi::ModernStream *file) {
    ...
    m->ph = (MOBIPdbHeader *)calloc(1, sizeof(MOBIPdbHeader));
    if (m->ph == NULL) {
...
MOBI_RET mobi_load_reclist(MOBIData *m, Libmobi::ModernStream *file) {
    ...
    m->rec = (MOBIPdbRecord *)calloc(1, sizeof(MOBIPdbRecord));
    if (m->rec == NULL) {
    ...
    for (int i = 0; i < m->ph->rec_count; i++) {
        ...
        if (i > 0) {
            curr->next = (MOBIPdbRecord *)calloc(1, sizeof(MOBIPdbRecord));
            if (curr->next == NULL) {
                debug_print("%s", "Memory allocation for pdb record failed\n");
                buffer_free(buf);
                return MOBI_MALLOC_FAILED;
...
MOBI_RET mobi_load_recdata(MOBIPdbRecord *rec, Libmobi::ModernStream *file) {
    ...
    rec->data = (unsigned char*)malloc(rec->size);
    if (rec->data == NULL) {
...
MOBI_RET mobi_parse_extheader(MOBIData *m, MOBIBuffer *buf) {
    ...
    m->eh = (MOBIExthHeader *)calloc(1, sizeof(MOBIExthHeader));
    if (m->eh == NULL) {
    ...
    for (size_t i = 0; i < rec_count; i++) {
        if (curr->data) {
            curr->next = (MOBIExthHeader *)calloc(1, sizeof(MOBIExthHeader));
            if (curr->next == NULL) {
                debug_print("%s", "Memory allocation for EXTH header failed\n");
                mobi_free_eh(m);
                return MOBI_MALLOC_FAILED;
        ...
        curr->tag = (MOBIExthTag)buffer_get32(buf);
...
MOBI_RET mobi_parse_mobiheader(MOBIData *m, MOBIBuffer *buf) {
    ...
    m->mh = (MOBIMobiHeader*)calloc(1, sizeof(MOBIMobiHeader));
    if (m->mh == NULL) {
    ...
    if (encoding == 1252) {
        m->mh->text_encoding = (MOBIEncoding*)malloc(sizeof(MOBIEncoding));
    ...
    if (m->mh->full_name_offset && m->mh->full_name_length) {
        ...
        m->mh->full_name = (char*)malloc(full_name_length + 1);
...
MOBI_RET mobi_parse_record0(MOBIData *m, const size_t seqnumber) {
    ...
    m->rh = (MOBIRecord0Header*)calloc(1, sizeof(MOBIRecord0Header));
    if (m->rh == NULL) {
...
MOBI_RET mobi_parse_cdic(MOBIHuffCdic *huffcdic, const MOBIPdbRecord *record, const size_t num) {
    ...
    if (num == 0) {
        ...
        huffcdic->symbol_offsets = (uint16_t*)malloc(index_count * sizeof(*huffcdic->symbol_offsets));
...
MOBI_RET mobi_parse_huffdic(const MOBIData *m, MOBIHuffCdic *huffcdic) {
    ...
    huffcdic->symbols = (unsigned char **)malloc((huff_rec_count - 1) * sizeof(*huffcdic->symbols));
...
MOBI_RET mobi_parse_fdst(const MOBIData *m, MOBIRawml *rawml) {
    ...
    rawml->fdst = (MOBIFdst*)malloc(sizeof(MOBIFdst));
    if (rawml->fdst == NULL) {
        debug_print("%s\n", "Memory allocation failed");
        buffer_free_null(buf);
        return MOBI_MALLOC_FAILED;
    ...
    rawml->fdst->fdst_section_starts = (uint32_t*)malloc(sizeof(*rawml->fdst->fdst_section_starts) * section_count);
    ...
    rawml->fdst->fdst_section_ends = (uint32_t*)malloc(sizeof(*rawml->fdst->fdst_section_ends) * section_count);
