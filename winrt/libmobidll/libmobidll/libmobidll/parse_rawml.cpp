...
MOBI_RET mobi_reconstruct_resources(const MOBIData *m, MOBIRawml *rawml) {
    ...
    while (curr_record != NULL) {
        ...
        MOBIPart *curr_part = (MOBIPart*)calloc(1, sizeof(MOBIPart));;
        if (curr_part == NULL) {
...
MOBI_RET mobi_reconstruct_flow(MOBIRawml *rawml, const char *text, const size_t length) {
    /* KF8 */
    if (rawml->fdst != NULL) {
        rawml->flow = (MOBIPart*)calloc(1, sizeof(MOBIPart));
        if (rawml->flow == NULL) {
        ...
        while (i < section_count) {
            if (i > 0) {
                curr->next = (MOBIPart*)calloc(1, sizeof(MOBIPart));
                if (curr->next == NULL) {
                    debug_print("%s", "Memory allocation for flow part failed\n");
                    return MOBI_MALLOC_FAILED;
                }
        ...
        if (memcmp(text, REPLICA_MAGIC, 4) == 0) {
            ...
            /* print replica */
            unsigned char *pdf = (unsigned char*)malloc(length);
            if (pdf == NULL) {
                debug_print("%s", "Memory allocation for flow part failed\n");
                return MOBI_MALLOC_FAILED;
            }
            ...
            section_data = (unsigned char*)malloc(section_length);
            if (section_data == NULL) {
                debug_print("%s", "Memory allocation failed\n");
                free(pdf);
                return MOBI_MALLOC_FAILED;
...
MOBI_RET mobi_reconstruct_parts(MOBIRawml *rawml) {
    ...
    rawml->markup = (MOBIPart*)calloc(1, sizeof(MOBIPart));
    ...
    if (rawml->skel == NULL) {
        unsigned char *data = (unsigned char*)malloc(buf->maxlen);
        if (data == NULL) {
            debug_print("%s", "Memory allocation failed\n");
            buffer_free_null(buf);
            return MOBI_MALLOC_FAILED;
    ...
    while (i < rawml->skel->entries_count) {
        ...
        char *skel_text = (char*)malloc(skel_length);
        if (skel_text == NULL) {
...
MOBI_RET mobi_reconstruct_orth(const MOBIRawml *rawml, MOBIFragment *first, size_t *new_size) {
    ...
    while (i < count) {
        ...
        char *entry_text;
        if (rawml->infl) {
            char *infl_tag = (char*)malloc(INDX_INFLTAG_SIZEMAX + 1);
            if (infl_tag == NULL) {
                debug_print("%s\n", "Memory allocation failed");
                mobi_trie_free(infl_trie);
                return MOBI_MALLOC_FAILED;
            ...
            entry_text = (char*)malloc(entry_length + 1);
            if (entry_text == NULL) {
                debug_print("%s\n", "Memory allocation failed");
                mobi_trie_free(infl_trie);
                free(infl_tag);
...
MOBI_RET mobi_markup_to_utf8(MOBIPart *part) {
    ...
    char *out_text = (char*)malloc(out_length);
    if (out_text == NULL) {
        debug_print("%s", "Memory allocation failed\n");
        return MOBI_MALLOC_FAILED;
    }
    ...
    text = (unsigned char*)malloc(out_length);
    if (text == NULL) {
        debug_print("%s", "Memory allocation failed\n");
        free(out_text);
        part->data = NULL;
...
MOBI_RET mobi_strip_mobitags(MOBIPart *part) {
    ...
    if (first) {
        ...
        unsigned char *new_data = (unsigned char*)malloc(part_size);
		if (new_data == NULL) {
			mobi_list_del_all(first);
			debug_print("%s\n", "Memory allocation failed");
			return MOBI_MALLOC_FAILED;
...
MOBI_RET mobi_parse_rawml_opt(MOBIRawml *rawml, const MOBIData *m, bool parse_toc, bool parse_dict, bool reconstruct) {
    ...
    char *text = (char*)malloc(maxlen + 1);
    if (text == NULL) {
        debug_print("%s", "Memory allocation failed\n");
        return MOBI_MALLOC_FAILED;
    }
