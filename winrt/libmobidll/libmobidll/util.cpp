...
MOBI_RET mobi_set_fullname(MOBIData *m, const char *fullname) {
    if (mobi_exists_mobiheader(m) && m->mh->full_name) {
        ...
        char *new_title = (char*)malloc(title_length + 1);
        if (new_title == NULL) {
...
MOBI_RET mobi_add_exthrecord(MOBIData *m, const MOBIExthTag tag, const uint32_t size, const void *value) {
    ...
    while (m && count--) {
        ...
        MOBIExthHeader *record = (MOBIExthHeader*)calloc(1, sizeof(MOBIExthHeader));
        if (record == NULL) {
        ...
        if (meta.type == EXTH_STRING && mobi_is_cp1252(m)) {
            ...
            MOBI_RET ret = mobi_utf8_to_cp1252(data, (char*)value, &data_size, size);
            if (ret != MOBI_SUCCESS) {
                free(record->data);
                free(record);
                free(data);
            ...
            MOBIBuffer *buf = buffer_init_null((unsigned char*)record->data, size);
            if (buf == NULL) {
                free(record->data);
                free(record);
                return MOBI_MALLOC_FAILED;
        ...
        if (m->eh == NULL) {
            if (m->mh->exth_flags == NULL) {
                m->mh->exth_flags = (uint32_t *)malloc(sizeof(uint32_t));
                if (m->mh->exth_flags == NULL) {
...
const MOBIExthMeta mobi_exth_tags[] = {
    ...
    /* end */
    {MOBIExthTag(0), MOBIExthType(0), NULL},
};
...
MOBIExthMeta mobi_get_exthtagmeta_by_tag(const MOBIExthTag tag) {
    ...
    return { MOBIExthTag(0), MOBIExthType(0), NULL};
}
...
char * mobi_decode_exthstring(const MOBIData *m, const unsigned char *data, const size_t size) {
    ...
    char *exth_string = (char*)malloc(out_length);
    if (exth_string == NULL) {
        debug_print("%s\n", "Memory allocation failed");
        return NULL;
    }
...
MOBI_RET mobi_decode_font_resource(unsigned char **decoded_font, size_t *decoded_size, MOBIPart *part) {
    ...
    *decoded_font = (unsigned char*)malloc(h.decoded_size);
...
size_t mobi_get_kf8boundary_seqnumber(const MOBIData *m) {
    ...
    if (exth_tag != NULL) {
        uint32_t rec_number = mobi_decode_exthvalue((unsigned char *)exth_tag->data, exth_tag->size);
        rec_number--;
...
void mobi_remove_zeros(unsigned char *buffer, size_t *len) {
    ...
    unsigned char *buf = (unsigned char*)memchr(buffer, 0, length);
    if (buf == NULL) {
        return;
    }
    buf++;
...
MOBI_RET mobi_swap_mobidata(MOBIData *m) {
    MOBIData *tmp = (MOBIData *)malloc(sizeof(MOBIData));
    if (tmp == NULL) {
