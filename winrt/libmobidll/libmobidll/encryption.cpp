...
static MOBI_RET mobi_pk1_decrypt(unsigned char *out, const unsigned char *in, size_t length, const unsigned char key[KEYSIZE]) {
    ...
    PK1 *pk1 = (PK1 *)calloc(1, sizeof(PK1));
    while (length--) {
...
static size_t mobi_drm_parse(MOBIDrm **drm, const MOBIData *m) {
    ...
    for (size_t i = 0; i < count; i++) {
        drm[i] = (MOBIDrm*)calloc(1, sizeof(MOBIDrm));
		if (drm[i] == NULL) {
			debug_print("%s\n", "Memory allocation failed");
			buffer_free_null(buf);
			return 0;
...
static MOBI_RET mobi_drm_getkey_v2(unsigned char key[KEYSIZE], const unsigned char *pid, const MOBIData *m) {
    ...
    MOBIDrm **drm = (MOBIDrm **)malloc(*m->mh->drm_count * sizeof(MOBIDrm*));
    if (drm == NULL) {
...
EXTHDrm * mobi_exthdrm_get(const MOBIData *m) {
    ...
    MOBIBuffer *buf = buffer_init_null((unsigned char *)meta->data, meta->size);
    if (buf == NULL) {
        return NULL;
    }
    ...
    while (buf->offset < buf->maxlen && submeta_count < ARRAYSIZE(submeta)) {
        ...
        const MOBIExthHeader *sub = mobi_get_exthrecord_by_tag(m,MOBIExthTag( exth_key));
        if (sub) {
    ...
    unsigned char *token = (unsigned char *)malloc(submeta_total);
    if (token == NULL) {
        buffer_free_null(buf);
        return NULL;
    }
    ...
    EXTHDrm *exth_drm = (EXTHDrm *)malloc(sizeof(EXTHDrm));
    if (exth_drm) {
        exth_drm->data =(unsigned char*) meta->data;
...
MOBI_RET mobi_drm_setkey_internal(MOBIData *m, const char *pid) {
    ...
    if (m->drm_key == NULL) {
        m->drm_key = (unsigned char *)malloc(KEYSIZE);
        if (m->drm_key == NULL) {
            debug_print("Memory allocation failed%s", "\n")
            return MOBI_MALLOC_FAILED;
        }
...
MOBI_RET mobi_drm_decrypt(MOBIData *m) {
    ...
    while (text_rec_count-- && curr) {
        ...
        unsigned char *decrypted = (unsigned char *)malloc(record_size);
        if (decrypted == NULL) {
            debug_print("Memory allocation failed%s", "\n");
            return MOBI_MALLOC_FAILED;
        }
